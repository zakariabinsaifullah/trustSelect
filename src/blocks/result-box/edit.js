 ...
 */
import { RichText, useBlockProps, MediaPlaceholder, InnerBlocks, BlockControls, MediaUpload } from '@wordpress/block-editor';
...
const Edit = props => {
    ...
    const cssCustomProperties = {
        ...
        ...(crossMarkColor && { '--crossmar-color': crossMarkColor }),
        ...(badgeColor && { '--badge-color': badgeColor }),
        ...(badgeBg && { '--badge-bg': badgeBg }),
        ...(gradientStart && { '--gradient-start': gradientStart }),
        ...(gradientEnd && { '--gradient-end': gradientEnd })
    };
            ...
            <div {...blockProps}>
                <div className="header-area gn-padding pb-0">
                    <div className="label-title-wrapper">
                        <RichText
                            ...
                            placeholder={__('label..', 'gutenbergnative-blocks')}
                        />
                        <RichText
                            className="title"
                            tagName={titleTag}
                            ...
                            placeholder={__('product title..', 'gutenbergnative-blocks')}
                        />
                        <RichText
                            tagName="div"
                            className="subtitle"
                            ...
                            placeholder={__('subtitle..', 'gutenbergnative-blocks')}
                        />
                    </div>
                    {showBadge && (
                ...
                <div className="gn-padding pt-0">
                    <RichText
                        ...
                        placeholder={__('write description..', 'gutenbergnative-blocks')}
                    />
                    <div className="price">
                        <RichText
                            className="price"
                            tagName="strong"
                            value={price?.label}
                            onChange={value => setAttributes({ price: { ...price, label: value } })}
                            placeholder={__('label', 'gutenbergnative-blocks')}
                        />
                        <RichText
                            className="price"
                            tagName="span"
                            value={price?.value}
                            onChange={value => setAttributes({ price: { ...price, value: value } })}
                            placeholder={__('value', 'gutenbergnative-blocks')}
                        />
                        <RichText
                            className="price"
                            tagName="span"
                            value={price?.currency}
                            onChange={value => setAttributes({ price: { ...price, currency: value } })}
                            placeholder={__('currency', 'gutenbergnative-blocks')}
                        />
                    </div>
                </div>
                                ...
                                return (
                                    <div className="score" key={index}>
                                        <div className="score-title">
                                            <span className="bar-title">{v?.title}</span>
                ...
                <div className="pros-cons-area gn-padding">
                    <div className="separator"></div>
                    <li className="pros-cons-headings list-item">
                        <div className="pros-heading pc-row">
                            <RichText
                                className="pc-heading"
                                value={proTitle}
                                onChange={value => setAttributes({ proTitle: value })}
                                placeholder={__('Vorteile', 'gutenbergnative-blocks-blocks')}
                            />
                        </div>
                        <div className="pc-heading cons-heading pc-row">
                            <RichText
                                ...
                                placeholder={__('Nachteiles', 'gutenbergnative-blocks-blocks')}
                            />
                        </div>
                    </li>
                                    ...
                                    {index < pros.length ? (
                                        <>
                                            ...
                                            <RichText
                                                ...
                                                placeholder={__('pros title', 'gutenbergnative-blocks')}
                                            />
                                            <button
                                                onClick={() => {
                                                    const newPros = [...pros];
                                    ...
                                    {index < cons.length ? (
                                        <>
                                            ...
                                            <RichText
                                                ...
                                                placeholder={__('cons title', 'gutenberg-native')}
                                            />
                                            <button
                                                onClick={() => {
                                                    const newPros = [...cons];
                                                    newPros.splice(index, 1);
                                                    setAttributes({ cons: newPros });
                                                }}
                                                className="remove"
                                            >
                    ...
                    <li className="pros-cons-action list-item">
                        ...
                        <div className="cons-action pc-row">
                            <AddControl
                                onClick={() => {
                                    setAttributes({
                                        cons: [...cons, 'new cons']
                    ...
                    <>
                        ...
                        <div className="conclusion-area gn-padding">
                            <RichText
                                ...
                                placeholder={__('Fazit Titel..', 'gutenbergnative-blocks')}
                            />
                            <InnerBlocks />
                            {showSecondaryBtn && (
